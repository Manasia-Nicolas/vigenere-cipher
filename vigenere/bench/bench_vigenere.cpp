...
//   g++ -std=c++23 -O2 vigenere/bench/bench_vigenere.cpp -o bench
//   ./bench
//
// For whole-program timing use hyperfine on the main binary instead:
//   hyperfine './vigenere en vigenere/long-encrypted.txt'
...
template <typename F>
static double TimeMs(F&& f, int repeats){
...
int main(){
    ...
    std::cout << std::left << std::setw(12) << "n"
              << std::setw(14) << "Kasiski(ms)"
              << std::setw(14) << "Friedman(ms)"
              << std::setw(16) << "FreqAnalysis(ms)" << "\n";

    for(int n : {1000, 5000, 20000, 80000, 320000}){
        std::string text(n, 'A');
        for(char& c : text) c = (char)('A' + letter(rng));
        ...
        double t_kas = TimeMs([&]{ VigenereCypher::FindLengthKasiski(letters, en.size()); }, repeats);
        double t_fri = TimeMs([&]{ VigenereCypher::FindLengthFriedman(letters, en.size()); }, repeats);
        double t_fa  = TimeMs([&]{ VigenereCypher::FrequentAnalysis(letters, 9, freq); }, repeats);

        std::cout << std::left << std::setw(12) << n
                  << std::setw(14) << std::fixed << std::setprecision(4) << t_kas
                  << std::setw(14) << t_fri
                  << std::setw(16) << t_fa << "\n";
    }
    return 0;
